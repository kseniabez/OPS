...
int read_message(int fd, char* message, int* op) {
    ...
    int size = atoi(buf);
    if(size <= 0) return -1;
    if(size > MAX_MESSAGE_SIZE) return -2;
...
void doLogger(int tcp_listen_socket, int log_file)
{
    ...
    char wrong_filter[18]="Wrong filter mode\n";
    char large_package[18]="Package too large\n";
    char reject_client[25]="Too much active clients\n";
    char buf[MAX_MESSAGE_SIZE];
        ...
        if ((nfds = epoll_pwait(epoll_descriptor, events, MAX_EVENTS, -1, &oldmask)) > 0)
        {
            for (int n = 0; n < nfds && do_work; n++)
            {
                ...
                if (port == -1 && fd == tcp_listen_socket) // New connection
                {
                    ...
                    {
                            if(bulk_write(fd, reject_client, strlen(reject_client)) < 0)
                            {
                                if(errno == EPIPE) {}
                                else ERR("write");
                            }
                ...
                else // Existing connection
                {
                    ...
                    if((c = read_message(fd, buf, &op)) <= 0)
                    {
                        ...
                        if(c==-2)
                        {
                            ...
                            if(bulk_write(fd, large_package, strlen(large_package)) < 0)
                            {
                                if(errno == EPIPE)
                                {
                                    int port = isConnected(client_sockets, fd);

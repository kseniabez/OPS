...
{
    printf("Logging: %s\n", message);
    if(bulk_write(fd, message, strlen(message)) < 0) ERR("write");
}
...
int main(int argc, char **argv)
{
    ...
    const int log_file = open(argv[2], O_WRONLY | O_CREAT, 0777);
    if (log_file == -1)
        ERR("open");
